...
struct AST {
  ...
  union {
    struct binop {
      enum binop_type { BIN_PLUS, BIN_MINUS, BIN_MUL , BIN_DEL } type;
...
static const char *BINOPS[] = {
    [BIN_PLUS] = "+", [BIN_MINUS] = "-", [BIN_MUL] = "*", [BIN_DEL] = "/"};
...
int main() {
  ...
  print(stdout, ast1);
  newline_print();
  struct AST *ast2 = mul(add(lit(3), lit(5)), del(lit(9), lit(7)));
  print(stdout, ast2);
  newline_print();
  return 0;
}